  ...
  for (const section in mergedData.properties) {
    for (const title in mergedData.properties[section]) {
      ...
      for (let i = 0; i < titleAsLines1.length; i++) {
        ...
        if (!exactMatchTypes.includes(currentType))
          continue;
      ...
      for (const removed of titleAsLines1) {
        if (exactMatchTypes.includes(removed.type))
          removedBuffer.push(removed);
      }
      for (const added of titleAsLines2) {
        if (exactMatchTypes.includes(added.type))
      ...
      if (matches.length > 0 || removedBuffer.length > 0 || addedBuffer.length > 0) {
        ...
        titleDiff.sort((x, y) => {
          ...
          let comparedValue2;
          if (x[0] == '~')
            comparedValue1 = x[1].lines[0][1];
          else
            comparedValue1 = x[1].lines[0];

          if (y[0] == '~')
            comparedValue2 = y[1].lines[0][1];
          else
            comparedValue2 = y[1].lines[0];
      ...
      for (const chunk in mergedData.properties[section][title]) {
        ...
        if (isChangeItem(chunkData)) {
          if (chunkData[0] == ' ') {
            ...
            for (let i = diffList[0][1].type == 'AXIOM' ? 1 : 0; i < diffList.length; i++) {
              ...
              if (match) {
                ...
                let index = diffList.indexOf(match);
                if(index !== -1) {
