...
process.env.VUE_APP_ONTOLOGY_VERSION = process.env.ONTOLOGY_VERSION || 'master/latest';

process.env.VUE_APP_PRODUCT = process.env.PRODUCT
...
process.env.VUE_BASE_URL = process.env.BASE_URL
  || `https://spec.${
    process.env.VUE_ONTOLOGY_NAME === 'idmp' ? 'pistoiaalliance' : 'edmcouncil'
  }.org/`;

process.env.VUE_RESOURCES_BASE_URL = `${
  process.env.VUE_BASE_URL + process.env.VUE_ONTOLOGY_NAME
}/ontology/`;
...
export default {
  ...
  generate: {
    ...
    routes() {
      return axios
        ...
        .then((res) => res.data.data.map((page) => {
  ...
  proxy: [
    ...
    [
      `/${process.env.VUE_ONTOLOGY_NAME}/strapi`,
      { target: process.env.STRAPI_URL, pathRewrite: { '^/[^/]+/strapi': '' } },
    ],
  ],
